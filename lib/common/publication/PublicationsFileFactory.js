...
export class PublicationsFileFactory {
    constructor(trustedCertificates = PUBLICATIONS_FILE_SIGNATURE_CONSTANTS.TrustedCertifiactes, signatueSubjectToVerify = PUBLICATIONS_FILE_SIGNATURE_CONSTANTS.GuardtimeSignatureSubjectEmail) {
    ...
    create(publicationFileBytes) {
        ...
        const verified = CMSVerification.verifyFromBytes(pubFile.getSignatureValue(), pubFile.getSignedBytes(), [this.trustedCertificates], this.signatueSubjectToVerify);
        if (!verified) {
