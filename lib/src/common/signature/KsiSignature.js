...
import { Base64Coder, UnsignedLongCoder } from 'gt-js-common';
// @ts-ignore
...
export class KsiSignature extends CompositeTag {
    ...
    getUuid() {
        const aggregationTimeBytes = UnsignedLongCoder.encode(this.getAggregationTime());
        const valueBytes = new Uint8Array(8).set(aggregationTimeBytes, 8 - aggregationTimeBytes.length);
        const linkBits = [];
        for (const chain of this.getAggregationHashChains()) {
        ...
        console.log(uuid(valueBytes, new Array(16)));
        return '';
    }
